...
def collate_catalog(a_string, history_colnames=[], halocat_propnames=[],
    ...
    is_orphan_mask = is_orphan_function(history_data['halo_id'])
...
def is_orphan_function(halo_id):
    return np.where(np.log10(halo_id) >= 13, True, False)
